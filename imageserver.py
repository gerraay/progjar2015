...
class MemprosesClient(threading.Thread):
	...
	def run(self):
		...
		while True:
				...
				print data
				if(re.search('GET /gambar1 HTTP/1.1', data)):
					self.client_socket.send(get_file('gambar1.jpg'))
					break
				if(re.search('GET /gambar2 HTTP/1.1', data)):
					self.client_socket.send(get_file('gambar2.jpg'))
					break
				if(re.search('GET /gambar3 HTTP/1.1', data)):
					self.client_socket.send(get_file('gambar3.jpg'))
					break
				if(re.search('GET /gambar4 HTTP/1.1', data)):
